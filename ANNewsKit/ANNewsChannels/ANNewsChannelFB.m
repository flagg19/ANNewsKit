 ...
 * necessarily the number of entries you will se, for two reasons:
 * - There might not be that much entries
 * - Response contains all public etries relative to the given entity, but the
 * local mapper will keep only entries that has the entity itself as author.
 */
    ...
    if (response) {
        ...
        for (NSDictionary* row in [response objectForKey:@"data"]) {
            ...
            if ([row objectForKey:@"message"] != nil) {
                // If the message was generated by the entity itself
                if ([[[row objectForKey:@"from"] objectForKey:@"id"] isEqualToString:self.entityId]) {
